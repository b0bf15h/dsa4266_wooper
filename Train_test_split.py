...
class Train_test_split(object):
    ...
    def train_test_split(self,train_size):
      grouped_gene_id=pd.Series(data.groupby(by='gene_id')['transcript_id'].count().sort_values())
      ...
      train_data=data[data['gene_id'].isin(train_gene_id)==True]
      test_data=data[data['gene_id'].isin(train_gene_id)!=True]
